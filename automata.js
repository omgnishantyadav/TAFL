...
export function simulateENFA(enfa, input) {
  ...
  for (const ch of input) {
    current = epsilonClosure(enfa, move(enfa, current, ch));
  }
  const accepted = [...current].some((s) => enfa.acceptStates.has(s));
  return { accepted, currentStates: current };
}
...
export function simulateDFA(dfa, input) {
  ...
  for (const ch of input) {
    ...
    if (!next || next.size === 0) return { accepted: false, haltedState: current };
    current = [...next][0];
  }
  return { accepted: dfa.acceptStates.has(current), haltedState: current };
}
